...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        with open(f"./processed_data/k_data.pkl", 'rb') as handle:
            k_data = pickle.load(handle)
        with open(f"./evaluation/test_discLP_data.pkl", 'rb') as handle:
            test_data_all = pickle.load(handle)
        with open(f"./evaluation/test_discLP_text.pkl", 'rb') as handle:
            text_all = pickle.load(handle)
        
        
        test_data_all = [torch.from_numpy(d).float() for d in test_data_all]
        id_tensor, motion_all, contLP_all, discLP_all, LP_weight_all, textemb_all, masks_tensor, progress_tensor = test_data_all
        id_tensor = id_tensor.long()
        masks_tensor = masks_tensor.bool()
        print(motion_all.shape, contLP_all.shape, textemb_all.shape)


        self.k_all = k_data #[k_data[int(i)] for i in id_tensor]
        # print("k_data", len(k_data), len(id_tensor))
        
        self.raw_text_list = text_all
        self.test_dataset = TensorDataset(id_tensor, motion_all, contLP_all, discLP_all, LP_weight_all,\
                                         textemb_all, masks_tensor, progress_tensor)
