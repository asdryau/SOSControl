...
#######
def smpl_to_humanml3d(trans, poses):
    # trans : (T,3)
    T = poses.shape[0]
...
if __name__ == "__main__":
    ...
    with open("./data/hml3d_motion_data.pkl", "rb") as file: 
        motion_dict = pickle.load(file)

    # process posetraj format
    motion_all = {}
    for k in motion_dict:
        poses, trans = motion_dict[k]
        if poses.shape[0] < 40:
            continue
        if poses.shape[0] > 197:
            poses = poses[:197].float()
            trans = trans[:197].float()
        print(k, poses.shape, trans.shape)
        motion_all[k] = smpl_to_humanml3d(trans, poses).detach()

    with open(f"./processed_data/posetraj_data.pkl", 'wb') as handle:
        pickle.dump(motion_all, handle, protocol=pickle.HIGHEST_PROTOCOL)
