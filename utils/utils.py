...
###
smpl = SMPL(
    model_path=Path("./utils/basicmodel_f_lbs_10_207_0_v1.1.0.pkl"),
    gender="FEMALE",
    batch_size=1,
    dtype=torch.float,
)
...
def save_motion(save_filename, root_traj, local_motion):
    ...
    np.savez(
        save_filename,
        poses=local_motion.reshape((T, -1)),
        trans=root_traj,
        betas=np.zeros(10),
        gender="female",
        mocap_framerate=30,
...
def forward_kinematics(root_traj, local_motion):
    ...
    local_motion = local_motion[:, :22]
    local_motion = torch.nn.functional.pad(local_motion, (0, 0, 0, 24 - J))  # 24 joints
    root_rot, poses = local_motion[:, 0], local_motion[:, 1:].reshape(
        (local_motion.shape[0], -1)
    )
    generated_smpl = smpl.forward(
        body_pose=poses, global_orient=root_rot, transl=root_traj, return_verts=False
    )
    joint_pos = generated_smpl.joints
    return joint_pos[:, :22]

def smpl_to_motion(trans, poses):
    # trans : (T,3)
    T = poses.shape[0]
