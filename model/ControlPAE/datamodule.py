...
# from utils.process_LP import spatial_LP_discretization, temporal_LP_unbaking
...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        with open(f"./processed_data/discLP_weight_data.pkl", 'rb') as handle:
            LP_weight_dict = pickle.load(handle)

        with open(f"./processed_data/discLP_full_data.pkl", 'rb') as handle:
            discLP_dict = pickle.load(handle)
        ...
        for k in motion_dict:
            ...
            if len_k > 196:
                len_k = 196
            motion_all.append(motion_dict[k])
            ...
            # also calc sparLP
            discLP_all.append(discLP_dict[k])
            # bp weight
            LP_weight = LP_weight_dict[k] / torch.amax(LP_weight_dict[k], dim=0, keepdims=True)
            LP_weight_all.append(LP_weight)
        ...
        # process length
        filter_motion_index = [i for i in range(len(rep_all)) if rep_all[i].shape[0] <= 196 or rep_all[i].shape[0] >= 40]
        motion_all = [motion_all[i] for i in range(len(motion_all)) if i in filter_motion_index]
        rep_all = [rep_all[i] for i in range(len(rep_all)) if i in filter_motion_index]
        discLP_all = [discLP_all[i] for i in range(len(discLP_all)) if i in filter_motion_index]
        LP_weight_all = [LP_weight_all[i] for i in range(len(LP_weight_all)) if i in filter_motion_index]
        ...
        discLP_all = pad_sequence(discLP_all, batch_first=True).float()
        LP_weight_all = pad_sequence(LP_weight_all, batch_first=True).float()
        ...
        motion_all = motion_all[:, :discLP_all.shape[1]]
        rep_all = rep_all[:, :discLP_all.shape[1]]
        masks_tensor = masks_tensor[:, :discLP_all.shape[1]]
        print(motion_all.shape, rep_all.shape, discLP_all.shape, LP_weight_all.shape, textemb_all.shape)
        

        # print("max class_index:", torch.amax(class_index_all))    #177
        self.train_dataset = TensorDataset(id_tensor[train_idx], motion_all[train_idx], rep_all[train_idx], discLP_all[train_idx], LP_weight_all[train_idx],\
                                           textemb_all[train_idx], masks_tensor[train_idx], progress_tensor[train_idx])
        self.val_dataset = TensorDataset(id_tensor[valid_idx], motion_all[valid_idx], rep_all[valid_idx], discLP_all[valid_idx], LP_weight_all[valid_idx],\
